...
class QueryTest extends FlatSpec {
  ...
  "Order" should "sort by all columns correctly" in {
    for(column <- StbSchema.instance.columns) {
      ...
      val result = Order(List(column)).apply(query.Filter(List()).apply(inputFile.iterator))

      for ((r, e) <- result.zip(expected.iterator)) {
  ...
  "Filter" should "filter empty rows; not filter matching rows" in {
    ...
    val result = query.Filter(List(("stb", "stb1"))).apply(inputFile.iterator)
    assert(result.size == expected.size)

    for((r, e) <- result.zip(expected.iterator)) {
      assert(r == e, "No filters.")
    }
  }
  ...
  it should "filter all rows with no matches" in {
    ...
    val result = query.Filter(List(("stb", "fajadlsfjskfja"))).apply(inputFile.iterator)
    assert(result.size == expected.size)
  }


  it should "leave input untouched without any filters" in {
    val result = query.Filter(List()).apply(inputFile.iterator)
  ...
  it should "compare big decimal correctly" in {
    ...
    val result = filter.apply(inputFile.iterator)
